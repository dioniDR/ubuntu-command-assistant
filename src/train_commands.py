...
def main(cfg: DictConfig):
    print(f"Iniciando entrenamiento de asistente de comandos Ubuntu")
    ...
    dataset_path = os.path.join(orig_cwd, "data/ubuntu_commands/commands_dataset.json")
    ...
    train_dataset = CommandsDataset(
        ...
        train_commands,
        tokenizer,
        max_length_input=cfg.model.max_length_input,
        max_length_output=cfg.model.max_length_output
    )
    ...
    train_loader = DataLoader(
        train_dataset,
        batch_size=min(cfg.training.batch_size, train_size),
        shuffle=True,
        num_workers=0
    )
    ...
    val_loader = DataLoader(
        eval_dataset,
        batch_size=min(cfg.training.batch_size, eval_size),
        shuffle=False,
        num_workers=0
    )
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath=os.path.join(orig_cwd, "outputs/ubuntu_assistant"),
    ...
    # Configurar logger
    logger = TensorBoardLogger(os.path.join(orig_cwd, "logs"), name="ubuntu_assistant")
    ...
    output_dir = os.path.join(orig_cwd, "outputs/ubuntu_assistant/final")
